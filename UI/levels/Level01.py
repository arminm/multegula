...
class Level01:
    def __init__(self, canvas_width, canvas_height):
        ...
        for x in range(1, 4):
           for y in range(Y_START, Y_END, BLOCK_HEIGHT*2):
                temp = Block(canvas_width, canvas_height, x*X_THIRD, y, PowerUps.PWR_NONE, Tilt.HORZ)
                self.blocks.append(temp)
                self.enabled.append(temp)
        # self.blocks.append(Block(canvas_width, canvas_height, X_THIRD, Y_START, PowerUps.PWR_NONE, Tilt.HORZ))
        # self.blocks.append(Block(canvas_width, canvas_height, 2*X_THIRD, Y_START, PowerUps.PWR_NONE, Tilt.HORZ))
        # self.blocks.append(Block(canvas_width, canvas_height, 3*X_THIRD, Y_START, PowerUps.PWR_NONE, Tilt.HORZ))
    ...
    def setBlocks(self, canvas):
        for block in self.blocks:
            temp = block.draw(canvas)
            block.setID(temp)

    def draw(self, canvas):
        if not(self.first) and self.updated:
            for block in self.blocks:
                canvas.delete(block.getID())
            ...
            for i, block in enumerate(self.blocks):
                if(block.getState() == True):
                    ...
                    if (ballBottom >= blkTop) and (ballTop < blkTop) and (blkLeft <= ballCenterX <= blkRight):
                        ...
                        canvas.data["ball"].setVelocity((-xVelocity), yVelocity)
                        self.blocks[i].disable();
                        self.updated = True;            # ball moving SOUTH EAST
